...
export default function ContactForm() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // Clear any global submission error when user starts editing
  ...
  const validateForm = () => {
    ...
    if (requestType === "booking") {
      if (!formData.date) {
        ...
        if (selectedDate < today) errors.date = "Appointment date cannot be in the past.";
      }
    }
    return errors;
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = requestType === "booking"
        ? { fullName: formData.fullName, phone: formData.phone, email: formData.email, serviceType: formData.serviceType, date: formData.date, timeSlot: formData.timeSlot, message: formData.message }
        : { fullName: formData.fullName, phone: formData.phone, email: formData.email, serviceType: formData.serviceType, message: formData.message };
      ...
      if (!res.ok) {
        ...
        throw new Error(data.message || data.error || "Something went wrong on our end.");
      }
      ...
      setSubmitError(err.message || "Unable to submit your request. Please try again or call us directly.");
    } finally {
    ...
    <section id="contact" className="py-24 bg-cream relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-2xl mx-auto">
          ...
          {/* Tab Toggle */}
          <div className="flex rounded-xl border border-zinc-200 overflow-hidden mb-8 bg-white shadow-sm">
            {(["quote", "booking"] as const).map((type) => (
              <button
                key={type}
                type="button"
                onClick={() => { setRequestType(type); setSubmitError(null); setFormErrors({}); }}
                className={`flex-1 py-3 px-4 text-sm font-semibold transition-all cursor-pointer ${requestType === type
                    ? "bg-[#1b3a2d] text-white"
                    : "text-zinc-600 hover:bg-zinc-50"
                  }`}
              >
                {type === "quote" ? "📋 Request a Quote" : "📅 Book a Crew"}
              </button>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            {!submitSuccess ? (
              <motion.form
                ...
                className="bg-white border border-zinc-200 rounded-2xl p-6 sm:p-8 shadow-sm space-y-5"
              >
                {/* Full Name */}
                <div id="field-fullName">
                  ...
                  <input
                    ...
                    name="fullName"
                    id="fullName"
                    placeholder="John Connelly"
                    value={formData.fullName}
                    onChange={handleInputChange}
                    className={`w-full bg-cream-light border text-zinc-800 placeholder-zinc-400 rounded-xl p-3 text-sm transition-all focus:outline-none ${formErrors.fullName ? "border-rose-500 focus:ring-1 focus:ring-rose-500" : "border-zinc-200 focus:border-primary focus:ring-1 focus:ring-primary"
                      }`}
                  />
                  {formErrors.fullName && (
                    <p className="text-xs text-rose-500 font-medium flex items-center mt-1.5">
                      <AlertCircle className="w-3.5 h-3.5 mr-1" />{formErrors.fullName}
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div id="field-phone">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      className={`w-full bg-cream-light border text-zinc-800 placeholder-zinc-400 rounded-xl p-3 text-sm transition-all focus:outline-none ${formErrors.phone ? "border-rose-500 focus:ring-1 focus:ring-rose-500" : "border-zinc-200 focus:border-primary focus:ring-1 focus:ring-primary"
                        }`}
                    />
                    {formErrors.phone && (
                      <p className="text-xs text-rose-500 font-medium flex items-center mt-1.5">
                        <AlertCircle className="w-3.5 h-3.5 mr-1" />{formErrors.phone}
                      </p>
                    )}
                  </div>
                  <div id="field-email">
                    ...
                    <input
                      ...
                      onChange={handleInputChange}
                      className={`w-full bg-cream-light border text-zinc-800 placeholder-zinc-400 rounded-xl p-3 text-sm transition-all focus:outline-none ${formErrors.email ? "border-rose-500 focus:ring-1 focus:ring-rose-500" : "border-zinc-200 focus:border-primary focus:ring-1 focus:ring-primary"
                        }`}
                    />
                    {formErrors.email && (
                      <p className="text-xs text-rose-500 font-medium flex items-center mt-1.5">
                        <AlertCircle className="w-3.5 h-3.5 mr-1" />{formErrors.email}
                      </p>
                    )}
                  </div>
                </div>
                ...
                {/* Service Type */}
                <div>
                  <label htmlFor="serviceType" className="block text-[10px] font-mono uppercase tracking-wider text-zinc-700 font-bold mb-1.5">
                  ...
                  >
                    <div id="field-date">
                      ...
                      <input
                        ...
                        value={formData.date}
                        onChange={handleInputChange}
                        className={`w-full bg-cream-light border text-zinc-800 rounded-xl p-3 text-sm transition-all focus:outline-none ${formErrors.date ? "border-rose-500 focus:ring-1 focus:ring-rose-500" : "border-zinc-200 focus:border-primary focus:ring-1 focus:ring-primary"
                          }`}
                      />
                      {formErrors.date && (
                        <p className="text-xs text-rose-500 font-medium flex items-center mt-1.5">
                          <AlertCircle className="w-3.5 h-3.5 mr-1" />{formErrors.date}
                        </p>
                      )}
                    </div>
                    <div>
                ...
                {submitError && (
                  <div className="flex items-start gap-2.5 bg-rose-50 border border-rose-200 rounded-xl p-3.5 text-sm text-rose-700">
                    <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
                    <div>
                      <p className="font-semibold">Submission failed</p>
                      <p className="font-normal text-rose-600">{submitError}</p>
                    </div>
                  </div>
                )}
